...
### Large delivery transport, observed M1

SentinelX read_media refused a137,769,298-byte source archive because its hub memory ceiling is100MiB. This is a transfer-size limit, not lost project access. Do not raise a security/memory limit or repeat the same oversized request. `python Tools/export_delivery_parts.py <archive> <new-parts-directory>` writes80MiB binary parts and PARTS.json without changing the archive. Retrieve each part through read_media, verify each hash, concatenate in manifest order in the container, verify the full original size/SHA256, and only then inspect/extract. The user receives one reassembled ordinary ZIP, not a manual reconstruction assignment. Preserve the immutable original and never reuse a parts directory.
