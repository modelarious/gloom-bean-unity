...
parts=['<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Gloom Bean Q11 — whole-game native review</title><style>'+css+'</style><body><main><header><div class="kicker">Native Unity / Q11 actor revision / 22 September 2026</div><h1>More expressive actors.<br>The whole game stays in view.</h1>',para('This is the actual built game, not a generated concept. Q11 retains the whole-stage materials and player-motion work and adds authored boss and patrol pose families driven by real state.'),'<div class="notice"><strong>The supplied concept-quality target is still not met.</strong> All 25 stages remain, including sparse, repetitive and poorly framed examples. Coverage and passing tests do not certify extreme visual fidelity.</div><div class="stats">']
...
parts+=['<h3>Later encounter footage — not just the opening</h3>',para('The first capped clips do not show an entire long encounter. These separately recorded sequences retain the later physical falling-mass release and the final Host adaptation/victory. They are actual foreground-owned game-window backbuffers, sampled approximately every 0.25 seconds. No camera or game state is staged.')]
late=json.loads((D/'LateMotion/MANIFEST.json').read_text())
for item in late['sequences']:
 name=item['name'];folder=ROOT/'Reports/Q11-LateAction'/name/'Frames';ss=item['files']
 parts+=['<article class="motion"><h3>'+E(name)+' — late action</h3>'+para(str(item['native_frames'])+' captured frames from '+f"{item['start']:.2f}"+' to '+f"{item['end']:.2f}"+' seconds; '+str(item['checks'])+' native route checks.')+'<div class="grid">']
 for k in [0,len(ss)//2,len(ss)-1]:parts.append(pic(folder/ss[k]['file'],f"Actual later action: {ss[k]['time']:.2f}s",True))
 video=D/'LateMotion'/item['video'];parts+=['</div><details><summary>Play later native sequence</summary><video controls preload="none" playsinline src="data:video/mp4;base64,'+base64.b64encode(video.read_bytes()).decode()+'"></video></details></article>']
parts+=['<div class="notice"><strong>Newly observed remaining defect:</strong> repeated instructional messages obscure too much of the upper playfield during real action. This is visible in the footage and is not hidden from the comparison. Foreground modules also remain repetitive. These are still outstanding; this delivery does not claim the supplied concept quality.</div>']
