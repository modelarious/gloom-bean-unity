...
$identity=Get-Content "$p/Delivery/WholeGame-Q8/PACKAGE_IDENTITY.json" -Raw|ConvertFrom-Json
$root="$p/Reports/Delivery-WholeGame-Q8"
...
try {
 ...
 do {$previous=Read-CampaignPredecessor -Path "$p/Reports/Orchard-v04/QualityBar-Q8-imported-regression/runner.json";if($previous.status -eq 'RUNNING'){if((Get-Date) -gt $deadline){throw 'Current test wait timeout'};Start-Sleep -Seconds 2}}while($previous.status -eq 'RUNNING')
 ...
 $status.endings=$ending.cases;$status.phase='actual-input-action-frames';Receipt
 foreach($spec in @(@{name='Laundry';flag='-gb-parish-verify -gb-route-id GB-L03';result='parish-result.json'},@{name='Mirror';flag='-gb-city-verify -gb-route-id GB-L09';result='city-result.json'},@{name='Scripture';flag='-gb-empyrean-verify -gb-route-id GB-L19 -gb-with-secrets';result='empyrean-result.json'})){
  $out=Join-Path $root $spec.name;New-Item -ItemType Directory $out|Out-Null
  $args=$spec.flag+' -gb-practice-witness -gb-action-captures -gb-render-fps 60 -gb-reports '+(Q $out)+' -logFile '+(Q "$out/player.log")+' -screen-width 1280 -screen-height 800 -screen-fullscreen 0'
  ...
  $action=Get-Content "$out/NativeAction/native-action.json" -Raw|ConvertFrom-Json
  if(@($action.shots).Count -lt 1){throw 'Missing actual native action capture'}
  $status.actions+=@{name=$spec.name;exit=$exit;failed=$result.failed;checks=$result.checks;native_frames=@($action.shots).Count;scope='Read-only observer of the real production-input route, no camera or actor staging'};Receipt
 }
