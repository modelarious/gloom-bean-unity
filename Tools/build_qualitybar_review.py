...
parts.append('<nav>'+ ' · '.join('<a href="#world'+str(w)+'">World '+str(w)+'</a>' for w in range(1,6))+' · <a href="#action">Actual play</a> · <a href="#references">All 52 references</a> · <a href="#method">Method / evidence</a></nav>')
...
parts.append('<h2 id="action">Actual play, not fixed-camera staging</h2><p>These 40 backbuffers were recorded during three production-input routes played in the exact extracted Windows ZIP. The observer only reads state: it does not reposition actors, freeze time or move the camera. Frames are sampled four seconds apart, not an animation-quality video.</p>')
ACTION=ROOT/'Reports/Delivery-WholeGame-Q8'
for name in ['Laundry','Mirror','Scripture']:
 manifest=json.loads((ACTION/name/'NativeAction/native-action.json').read_text())
 parts.append('<details><summary>'+esc(name)+' — all '+str(len(manifest['shots']))+' actual-action frames</summary><div class="grid">')
 for sh in manifest['shots']:parts.append(image(ACTION/name/'NativeAction'/sh['file'],f"{sh['time']:.1f}s / {sh['form']} / {sh['phase']}",True))
 parts.append('</div></details>')
parts.append('<details><summary>Actual keyboard journey — all 14 menu / gameplay / pause / help frames</summary><div class="grid">')
ui=json.loads((ACTION/'KeyboardUI/ui-result.json').read_text(encoding='utf-8-sig'))
for name in ui['screens']:parts.append(image(ACTION/'KeyboardUI'/(name+'.png'),name,True))
parts.append('</div></details><details><summary>Exact-package ending screens (spoilers): 0 / 19 / 20 secrets</summary><div class="grid triple">')
for name in ['Ordinary','Nineteen','Restored']:parts.append(image(ACTION/'Endings'/name/'ending-screen.png',name+' ending after real boss victory',True))
parts.append('</div></details>')
