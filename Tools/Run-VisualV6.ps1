...
try {
 ...
 $proc=Start-Process "$p/Builds/Windows/GloomBean.exe" -ArgumentList ('-gb-visual-v6 -gb-reports "'+$out+'" -logFile "'+$out+'/player.log" -screen-width 1280 -screen-height 800 -screen-fullscreen 0') -PassThru
 if(-not $proc.WaitForExit(90000)){Stop-Process -Id $proc.Id -Force;throw 'Visual fixture timeout'}
 $proc.Refresh();$status.exit=$proc.ExitCode
 $r=Get-Content "$out/visual-result.json" -Raw|ConvertFrom-Json
 $status.images=$r.images;$status.status=if($proc.ExitCode -eq 0 -and $r.status -eq 'PASS'){'PASS'}else{'FAIL'}
