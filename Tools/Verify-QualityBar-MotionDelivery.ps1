...
$identity=Get-Content "$p/Delivery/WholeGame-Q10/PACKAGE_IDENTITY.json" -Raw|ConvertFrom-Json
$root="$p/Reports/Delivery-WholeGame-Q10"
...
try {
 ...
 do {$previous=Read-CampaignPredecessor -Path "$p/Reports/Orchard-v04/QualityBar-Q10B-regression/runner.json";if($previous.status -eq 'RUNNING'){if((Get-Date) -gt $deadline){throw 'Current test wait timeout'};Start-Sleep -Seconds 2}}while($previous.status -eq 'RUNNING')
 ...
 foreach($spec in @(@{name='Laundry';flag='-gb-parish-verify -gb-route-id GB-L03';result='parish-result.json'},@{name='Mirror';flag='-gb-city-verify -gb-route-id GB-L09';result='city-result.json'},@{name='RootDitch';flag='-gb-orchard-verify -gb-route-id GB-L07';result='orchard-result.json'},@{name='Weight';flag='-gb-fall-verify -gb-route-id GB-B4';result='fall-result.json'})){
  ...
  if(@($action.shots).Count -lt 12){throw 'Missing actual native motion sequence'}
