...
try {
 ...
 foreach($c in $cases){
  ...
  $mode=switch($c.suite){'Mechanics'{'-gb-verify'} 'OpeningRoute'{'-gb-route-verify'} 'Orchard'{'-gb-orchard-verify -gb-route-id '+$c.route} 'City'{'-gb-city-verify -gb-route-id '+$c.route} default {'-gb-parish-verify -gb-route-id '+$c.route}}
 ...
 foreach($j in $jobs){
  ...
  $file=switch($c.suite){'Mechanics'{'verification.json'} 'OpeningRoute'{'route-result.json'} 'Orchard'{'orchard-result.json'} 'City'{'city-result.json'} default {'parish-result.json'}}
