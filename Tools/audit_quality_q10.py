"""Independent Q10 coverage/source audit; it cannot certify beauty or animation feel."""
...
newfiles=[f for f in after if f not in before];assert {Path(f).name for f in newfiles}<={'QualityBarConstruction.cs','QualityBarMotion.cs','QualityBarMotionChecks.cs','QualityBarLivingMechanisms.cs','QualityBarLivingChecks.cs'}
...
out=ROOT/'Documentation/QualityBarQ1'/('AUDIT_'+args.run+'.json');out.write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8');print('WHOLE_GAME_Q10_AUDIT_PASS',len(images),'images;',len(new),'new positions;',len(protected),'unchanged gameplay sources;',categories)
