"""Independent Q11 coverage/source audit; it cannot certify beauty or animation feel."""
from pathlib import Path
from collections import Counter
import sys,hashlib,json,math,subprocess,importlib.util,argparse
ROOT=Path(__file__).resolve().parents[1];sys.path[:0]=[str(ROOT/'.visual-tools'),str(ROOT.parent/'GloomBeanUnity/.visual-tools'),r'C:\Users\micha\Projects\GloomBeanUnity\.visual-tools']
...
out=ROOT/'Documentation/QualityBarQ1'/('AUDIT_'+args.run+'.json');out.write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8');print('WHOLE_GAME_Q11_AUDIT_PASS',len(images),'images;',len(new),'new positions;',len(protected),'unchanged gameplay sources;',categories)
