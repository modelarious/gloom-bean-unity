...
function Key([byte]$key,[int]$hold=100){Focus;$scan=[byte][GloomWindow]::MapVirtualKey($key,0);$extended=if($key -in @(37,38,39,40)){1}else{0};[GloomWindow]::keybd_event($key,$scan,$extended,[UIntPtr]::Zero);try{Start-Sleep -Milliseconds $hold}finally{[GloomWindow]::keybd_event($key,$scan,($extended -bor 2),[UIntPtr]::Zero)};$status.actions+=@{key=$key;milliseconds=$hold};Start-Sleep -Milliseconds 350}
