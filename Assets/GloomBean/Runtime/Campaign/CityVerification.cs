...
namespace GloomBean.Campaign
{
    ...
    public sealed class CityVerification:MonoBehaviour
    {
        ...
        IEnumerator Hotel(bool secret)
        {
            ...
            if(secret&&!stopped){yield return Walk(47);yield return Jump(47,33.9f);var lift=session.GetComponentsInChildren<ApartmentLift>().First();Check("board the actual service winch",actor.GroundCollider&&actor.GroundCollider.attachedRigidbody==lift.GetComponent<Rigidbody2D>());yield return Press(new InputFrame{interact=true});
                yield return Await("chandelier winch physically docks at its back",()=>Vector2.Distance(lift.transform.position,lift.upper)<.1f&&actor.Grounded&&actor.GroundCollider&&actor.GroundCollider.attachedRigidbody==lift.GetComponent<Rigidbody2D>(),8,()=>new InputFrame{move=new Vector2(Mathf.Clamp(lift.transform.position.x-actor.Body.position.x,-1,1),0)});
                yield return Walk(33.1f);Check("reveal inside of lobby chandelier",host.Has(HostKind.InsideOut));yield return Walk(38);Check("lobby chandelier Mercy collected",session.Mercies.Count==1);Snapshot("chandelier-interior");yield return Walk(30.8f);yield return Await("chandelier frame restores exterior body",()=>!host.Has(HostKind.InsideOut),3);yield return Walk(31.5f);yield return Await("reboard actual winch after leaving chandelier",()=>actor.GroundCollider&&actor.GroundCollider.attachedRigidbody==lift.GetComponent<Rigidbody2D>(),3);yield return Press(new InputFrame{interact=true});
                yield return Await("winch returns through same physical space",()=>Vector2.Distance(lift.transform.position,lift.lower)<.1f&&actor.Grounded&&actor.GroundCollider&&actor.GroundCollider.attachedRigidbody==lift.GetComponent<Rigidbody2D>(),8,()=>new InputFrame{move=new Vector2(Mathf.Clamp(lift.transform.position.x-actor.Body.position.x,-1,1),0)});}
