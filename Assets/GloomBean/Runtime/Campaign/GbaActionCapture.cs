...
namespace GloomBean.Campaign
{
    ...
    {
        [Serializable] public class Shot {public string file,stage,form,phase,screen,input;public float time,x,y,cameraSize;public bool motorEnabled,dynamicBody,cameraFollow;}
        ...
        GameRoot game;string directory;readonly List<Shot> shots=new List<Shot>();
        public static void Install(GameRoot root){if(root.testMode&&Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-action-captures")>=0){var capture=root.gameObject.AddComponent<GbaActionCapture>();capture.game=root;}}
        IEnumerator Start()
        {
            directory=Path.Combine(game.reportDirectory,"NativeAction");Directory.CreateDirectory(directory);
            while(shots.Count<30){yield return new WaitForSecondsRealtime(4);
