...
namespace GloomBean.Campaign
{
    ...
    public sealed class OrchardVerification:MonoBehaviour
    {
        ...
        void TraceEvent(string kind,string detail){if(kind=="pump"||kind=="fruit-burst"||kind.StartsWith("terrain-")||kind=="boss-phase")Note("EVENT t="+Time.time.ToString("0.00")+" "+kind+" "+detail);}
        ...
        IEnumerator Debris(float x)
        {
            if(stopped||!Live)yield break;float end=Time.time+40,blocked=0,lastJump=-9;
            while(Live&&Time.time<end&&Mathf.Abs(actor.Body.position.x-x)>.25f){float dx=x-actor.Body.position.x;
                blocked=Mathf.Abs(actor.Body.linearVelocity.x)<.7f?blocked+Time.fixedDeltaTime:0;
                bool jump=blocked>.3f&&actor.Grounded&&Time.time-lastJump>1;if(jump)lastJump=Time.time;
                input.frame=new InputFrame{move=new Vector2(Mathf.Clamp(dx,-1,1),0),run=true,jump=jump,jumpHeld=true,attack=jump};yield return NextPhysics();}
            input.frame=default;Check("run and jump through actual fallen debris",Mathf.Abs(actor.Body.position.x-x)<.7f);
        }
        ...
        {
            if(stopped||!Live)yield break;yield return Focus(HostKind.Root);var root=host.Form<RootForm>();Check("Root acquired before soil route",root!=null);if(stopped)yield break;
        ...
        IEnumerator Pears(bool secret)
        {
            ...
            yield return Walk(26);yield return Await("diverted wax physically rises",()=>actor.Feet.y>3.5f,5);yield return Walk(30);yield return Melt(false);yield return Await("land in pear cage",()=>actor.Grounded,5);Snapshot("wax-flow");
            if(secret){yield return Walk(32);yield return Jump(35,4.8f);yield return Press(new InputFrame{alternate=true,move=Vector2.right});Check("second partition changes scale mass",Mathf.Abs(host.waxVolume-.56f)<.01f);yield return Jump(38,6.3f);yield return Walk(39);var shutter=session.GetComponentsInChildren<MassWindow>().First();yield return Await("fine mass balance opens glass bell",()=>shutter.Valid,4);yield return Walk(44);Check("Glass bell Mercy collected",session.Mercies.Count==1);Snapshot("mass-balance");yield return Press(new InputFrame{interact=true});yield return Walk(32);yield return Await("leave secret on main bough",()=>actor.Grounded&&actor.Feet.y<4.3f,5);}
            ...
            yield return Walk(66);yield return Await("drop between boughs to the maintenance return",()=>actor.Grounded&&actor.Feet.y<.3f,5);yield return Melt(true);yield return Walk(30);Check("liquid return passes beneath the sagging bough",host.Form<WaxForm>().Liquid&&actor.Body.position.x<31);yield return Melt(false);yield return Walk(20);yield return Press(new InputFrame{move=Vector2.left,attack=true});yield return Pause(.4f);yield return Melt(true);yield return Walk(5);yield return Walk(2);
        }
        ...
        IEnumerator Kitchen(bool secret)
        {
            ...
            yield return Jump(28,-1);yield return Hop(33,-1.1f,-.7f);yield return Press(new InputFrame{jump=true,jumpHeld=true,move=Vector2.right});yield return Await("jump into the steam column",()=>actor.Body.position.x>36,4,()=>new InputFrame{move=Vector2.right,jumpHeld=true});
            ...
            if(secret){yield return Eat(Vector2.down);yield return Walk(43.8f);yield return Spit(Vector2.right);yield return Hop(47,7.5f,13.3f);var freight=session.GetComponentInChildren<TerrainServingLift>();yield return Await("actual terrain rides the freight dish",()=>freight.transform.position.y>11.8f,6);yield return Hop(52,12.8f,13.2f);Check("Kitchen Mercy",session.Mercies.Count==1);Snapshot("terrain-freight");yield return Walk(50.2f);yield return Hop(46,13.1f,13.7f,true);yield return Walk(43.5f);yield return Await("leave the delivered terrain by its open edge",()=>actor.Grounded&&actor.Feet.y<8.2f,6);}
            yield return Walk(44.5f);yield return Await("reach kitchen lower counter",()=>actor.Grounded&&actor.Feet.y<=5.1f,6);yield return Walk(43.3f);yield return Await("service floor",()=>actor.Grounded&&actor.Feet.y<3.3f,5);yield return Walk(46);Check("votive source combines Wax with Gullet",host.Has(HostKind.Wax));
            yield return Focus(HostKind.Wax);yield return Melt(true);yield return Walk(57);yield return Melt(false);yield return Walk(64);yield return Jump(67,1.8f);yield return Walk(68.2f);yield return Jump(72,3.6f);yield return Walk(73.2f);yield return Jump(77,5.4f);yield return Walk(78.2f);yield return Jump(82,7.2f);Check("physical kitchen key",session.HasKey);
            yield return Walk(83.6f);yield return Press(new InputFrame{interact=true});Check("hatches reverse",session.Phase==RunPhase.Returning);Snapshot("turn");yield return Walk(85.2f);yield return Jump(88,9);yield return Press(new InputFrame{interact=true});yield return Jump(86,10.8f);yield return Walk(-2,true);yield return Await("return gallery descends to entry",()=>actor.Grounded&&actor.Feet.y<2.3f,6);yield return Walk(2);
        }
        IEnumerator Ditch(bool secret)
        {
            ...
            yield return Walk(65);yield return Eat(Vector2.right);var sluice=session.GetComponentInChildren<MaterialSluice>();yield return Await("removing real drain terrain dries the used seam",()=>!sluice.Wet&&!sluice.channel[1].wet,2);Snapshot("drained-seam");yield return Walk(70);yield return RootPath(new Vector2(70,-4),new Vector2(81,-4),new Vector2(81,1),new Vector2(85,1));
            yield return Jump(90,2);yield return Jump(94,4);yield return Hop(99,5.8f,6.2f,true);Check("irrigation key",session.HasKey);yield return Walk(105.6f);yield return Press(new InputFrame{interact=true});Check("reverse local pump",session.Phase==RunPhase.Returning);yield return Await("reversed flow restores the drained return seam",()=>sluice.Wet&&sluice.channel[1].wet,2);Snapshot("turn");
            ...
            yield return Walk(48.5f);yield return RootPath(new Vector2(44,1),new Vector2(44,-4.8f),new Vector2(34,-4.8f),new Vector2(34,1));
        ...
        IEnumerator Seasons(bool secret)
        {
            ...
            yield return Walk(30.2f);yield return Hop(34,.4f,.8f,true);yield return Walk(34.8f);yield return Hop(39,-.1f,.1f,true);yield return Walk(43);yield return Eat(Vector2.down);yield return Await("pantry catches the displaced floor",()=>actor.Grounded&&actor.Feet.y< -1.8f,5);yield return Jump(45.5f,-.7f);yield return Hop(49,-.1f,.1f);
            yield return Walk(55);yield return Focus(HostKind.Root);
            if(secret){yield return RootPath(new Vector2(55,7),new Vector2(63,7),new Vector2(63,10));Check("Spring crown Mercy",session.Mercies.Count==1);Snapshot("spring-crown");yield return Walk(55);yield return Await("back to root entrance",()=>actor.Grounded&&actor.Feet.y<.2f,8);}
            ...
            yield return Hop(69,.8f,1.2f);yield return Hop(74,-.1f,4.5f);yield return Hop(78,.1f,4.5f);yield return Walk(79.3f);yield return Hop(84,.1f,4.8f,true);yield return Walk(85.3f);yield return Hop(90,.1f,5,true);yield return Walk(91.3f);yield return Hop(96,.1f,5.5f,true);yield return Walk(97.3f);yield return Hop(101,1.8f,2.2f,true);yield return Jump(105,4);yield return Walk(106.8f);Check("season key",session.HasKey);yield return Hop(113,5.8f,6.2f,true);
            ...
            yield return Walk(104);yield return Hop(96,.1f,5.5f,true);yield return Hop(90,.1f,5.5f,true);yield return Hop(84,.1f,5.5f,true);yield return Hop(78,.1f,5.5f,true);yield return Hop(74,-.1f,4,true);yield return Hop(69,.8f,1.2f);yield return Hop(65.5f,-.1f,.1f);yield return Focus(HostKind.Root);yield return RootPath(new Vector2(65,-4),new Vector2(55,-4),new Vector2(55,1));
            yield return Walk(47.8f);yield return Hop(41,-.1f,.1f,true);yield return Walk(39);yield return Hop(34,.4f,.8f,true);yield return Hop(29,-.1f,.1f,true);yield return Walk(20);yield return Walk(18.5f);yield return Press(new InputFrame{interact=true});
        ...
        IEnumerator Judge()
        {
            ...
            int phaseHealth=actor.Health;yield return Await("Judge act II: unsupported mass falls",()=>boss.phase>=2,6);Check("Judge arena transition does not refill health",actor.Health<=phaseHealth);yield return Walk(24);yield return RootPath(new Vector2(24,-4),new Vector2(37,-4),new Vector2(37,1));yield return Walk(39);Check("Judge defeated through material systems",boss.defeated&&session.Phase==RunPhase.Cleared);
        }
        IEnumerator Run()
        {
            game.SelectSource(1);var world=game.AvailableWorlds[1];
            if(Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-require-earned-world")>=0&&!CampaignProgression.WorldOpen(game.AvailableWorlds,1,game.Save.Data,false)){failures++;Note("FAIL Orchard was not earned by the supplied real save");Finish();yield break;}string selected=Arg("-gb-route-id","W2");bool secrets=Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-with-secrets")>=0;
        ...
        void Finish()
        {
            ...
            File.WriteAllText(Path.Combine(dir,"saved-progress.json"),JsonUtility.ToJson(game.Save.Data,true));
