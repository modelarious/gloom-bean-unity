...
namespace GloomBean.Campaign
{
    ...
    public sealed class BroadVisualCapture:MonoBehaviour
    {
        ...
        IEnumerator Run(){QualitySettings.vSyncCount=0;Application.targetFrameRate=60;
            ...
            foreach(var stage in stages)for(int sample=0;sample<3;sample++){
                ...
                int index=Mathf.Clamp(Mathf.FloorToInt(candidates.Length*new[]{.22f,.63f,.88f}[sample]),0,candidates.Length-1);var anchor=candidates[index];
            ...
            File.WriteAllText(Path.Combine(dir,"broad-result.json"),JsonUtility.ToJson(new Receipt{status=errors.Count==0&&shots.Count==75?"PASS":"FAIL",scope="75 NEW deterministic support-quantile native images across all25stages. A/B are review, C is held-out. Staged camera/body/form; not reachable-route proof. No old frame reuse.",shots=shots.ToArray(),errors=errors.ToArray()},true));
