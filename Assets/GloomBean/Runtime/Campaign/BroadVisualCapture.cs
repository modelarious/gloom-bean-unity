...
namespace GloomBean.Campaign
{
    ...
    public sealed class BroadVisualCapture:MonoBehaviour
    {
        ...
        IEnumerator Run(){QualitySettings.vSyncCount=0;Application.targetFrameRate=60;
            ...
            foreach(var stage in stages)for(int sample=0;sample<4;sample++){
                ...
                int index=Mathf.Clamp(Mathf.FloorToInt(candidates.Length*new[]{.22f,.63f,.88f,.43f}[sample]),0,candidates.Length-1);var anchor=candidates[index];
                ...
                string id=stage.id+"-"+(sample==0?"A-explore":sample==1?"B-altered":sample==2?"C-holdout":"D-validation");var image=ScreenCapture.CaptureScreenshotAsTexture();File.WriteAllBytes(Path.Combine(dir,id+".png"),image.EncodeToPNG());Destroy(image);
            ...
            File.WriteAllText(Path.Combine(dir,"broad-result.json"),JsonUtility.ToJson(new Receipt{status=errors.Count==0&&shots.Count==100?"PASS":"FAIL",scope="100 NEW native images across all25stages. A/B reviewed; C withheld through3passes. D is fresh validation after shared C-findings, with full boss-body framing. Staged camera/body/form; not reachable-route proof. No old frame reuse.",shots=shots.ToArray(),errors=errors.ToArray()},true));
