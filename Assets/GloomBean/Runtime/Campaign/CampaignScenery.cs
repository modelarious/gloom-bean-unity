...
namespace GloomBean.Campaign
{
    ...
    public sealed class CampaignScenery:MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            for(int i=0;i<3;i++){pictures[i].transform.position=new Vector3(c.x-phase+(i-1)*width,c.y,2);pictures[i].transform.localScale=new Vector3(scale,scale,1);pictures[i].color=theme==0?Color.white:new Color(.93f,.88f,.97f,1);}
        }
    }
