...
namespace GloomBean.Campaign
{
    ...
    public sealed partial class EmpyreanVerification:MonoBehaviour
    {
        ...
        IEnumerator Flight(Vector2 destination,Rigidbody2D landingBody=null)
        {
            ...
            input.rule=()=>{
                ...
                int pole=landingBody?(pos.x>landingBody.position.x-1.8f?-1:1):(session.Phase==RunPhase.Returning&&destination.x==71&&pos.x<80?-1:orbitalContact?(Vector2.Dot(north,wanted)>=0?1:-1):1);bool toggle=pole!=magnet.Polarity&&Time.fixedTime>=nextToggle;if(toggle)nextToggle=Time.fixedTime+.12f;
        ...
        IEnumerator BellScreenRoute(bool returning)
        {
            ...
            if(!returning){
                ...
                float end=Time.time+20,trace=0;bool atGuide=false;input.rule=()=>{
                    if(actor.Body.position.x<81.03f)atGuide=true;
                    // Repel to the visible guide, then attract while the guide supplies the reaction.
                    int pole=!atGuide&&cable.bell.position.y>13?1:-1;
                    float command=cable.bell.position.y>13?-1:Mathf.Clamp((83-actor.Body.position.x)*4-actor.Body.linearVelocity.x,-1,1);
                    return new InputFrame{action=magnet.Polarity!=pole,move=new Vector2(command,0)};
                };
                ...
                Check("falling bell lifts screen and Host through the cable",cable.deck.position.y>18.6f&&actor.Feet.y>18&&cable.bell.position.y<12&&cable.PeakTension>10);Snapshot("bell-screen-exchange");
...
input.rule=null;input.frame=default;
        ...
        IEnumerator Run()
        {
            ...
            var definition=route==world.boss.id?world.boss:world.levels.FirstOrDefault(d=>d.id==route);
            if(definition==null||(!definition.boss&&definition.course!=17&&definition.course!=18&&definition.course!=19&&definition.course!=20)){failures++;Note("FAIL No complete input witness authored for "+route+". This is not a campaign success.");Finish();yield break;}
            bool requireEarned=Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-require-earned-world")>=0;
            if(requireEarned&&!(definition.boss?CampaignProgression.BossOpen(world,game.Save.Data,false):CampaignProgression.LevelOpen(world,Array.IndexOf(world.levels,definition),game.Save.Data,false))){failures++;Note("FAIL Empyrean stage was not earned by the supplied real save");Finish();yield break;}
            ...
            Note("BEGIN "+definition.id+" "+definition.title);float startDelay;float.TryParse(Arg("-gb-start-delay","0"),System.Globalization.NumberStyles.Float,System.Globalization.CultureInfo.InvariantCulture,out startDelay);yield return Pause(.35f+Mathf.Clamp(startDelay,0,20));if(Arg("-gb-rail-probe","0")=="1"){yield return RailProbe();Finish();yield break;}if(Arg("-gb-scripture-no-ink","0")=="1"){yield return ScriptureNoInkControl();File.WriteAllText(Path.Combine(dir,"SCOPE.txt"),"Causal input-only negative control; not a level completion.");Finish();yield break;}if(definition.boss)yield return HostBossRoute();else if(definition.course==17)yield return Halos(secrets);else if(definition.course==18)yield return NoShadows(secrets);else if(definition.course==19)yield return ScriptureRoute(secrets);else yield return WhiteGateRoute(secrets);
            if(!stopped){Check("physical return completed",session.Phase==RunPhase.Cleared);if(!stopped){Check(Practice?"practice does not award progress":"clear is persistent",Practice?!game.Save.Data.cleared.Contains(route):game.Save.Data.cleared.Contains(route));if(!Practice&&!definition.boss)Check(secrets?"Mercy is saved":"Mercy was optional",secrets?game.Save.Data.mercies.Contains(route+"-MERCY"):session.Mercies.Count==0);}}
            if(!stopped&&Practice)Check("practice never saves its optional Mercy",!game.Save.Data.mercies.Contains(route+"-MERCY"));
            ...
            if(!stopped&&definition.boss){
                ...
                var presentation=game.GetComponent<CampaignPresentation>();Note("ENDING DISPLAY batch="+Application.isBatchMode+" actual="+(presentation?presentation.LastPaintedScreen:"missing")+" normal="+(presentation&&presentation.LastFigureWasNormal));Check("ending artwork obeys the real save and practice boundary",presentation&&presentation.LastPaintedScreen=="Ending"&&presentation.LastFigureWasNormal==(!Practice&&reloaded.RestoredEnding));
