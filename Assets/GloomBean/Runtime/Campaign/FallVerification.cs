...
namespace GloomBean.Campaign
{
    ...
    public sealed class FallVerification:MonoBehaviour
    {
        ...
        void Check(string name,bool pass,bool stop=true){assertions++;if(!pass){failures++;if(stop&&!stopped){Snapshot("first-failure");var coffinDebug=host.Form<CoffinForm>();if(coffinDebug!=null)Note("COFFIN blocker="+coffinDebug.LastBlocker+" hull="+coffinDebug.Hull.bounds+" angle="+actor.Body.rotation+" ground="+actor.GroundCollider);var bossMass=session.GetComponentInChildren<ColossusMass>();if(bossMass)Note("COLOSSUS position="+bossMass.Body.position+" velocity="+bossMass.Body.linearVelocity+" contact="+bossMass.LastContact+" phase="+session.GetComponentInChildren<AtlasBoss>().phase);Note("PHYSICAL shape="+actor.Shape.bounds+" axis="+actor.Shape.direction+" forms="+string.Join(",",host.Forms.Select(f=>f.Kind.ToString())));
        ...
        IEnumerator SeamBridge(bool secret)
        {
            ...
            if(secret){yield return Fold("island",Vector2.up,35.6f);yield return Jump(39,10.6f);yield return Leap(41.6f,11.8f,13.6f,false);yield return Walk(45);yield return Leap(49.5f,15.5f,15.9f,false);Check("Mercy island moves with folded support",session.GetComponentInChildren<FoldTipIsland>().transform.position.y<17);Check("suspended island Mercy",session.Mercies.Count==1);yield return Await("real run-up on the moved island",()=>actor.Body.position.x>=51.2f,2,()=>new InputFrame{move=Vector2.right,run=true});
        ...
        IEnumerator Freefall(bool secret)
        {
            ...
            if(secret){var chapel=session.GetComponentsInChildren<MotionPlatform>().First(x=>x.name=="Passing Mercy chapel");yield return Await("chapel aligns for the limited detour",()=>chapel.transform.position.x<50.3f,18);yield return Board(chapel);
                yield return Await("passing chapel Mercy",()=>session.Mercies.Count==1,5,()=>{var mercy=session.GetComponentsInChildren<Pickup>().FirstOrDefault(x=>x.kind==PickupKind.Mercy);return new InputFrame{move=mercy?new Vector2(Mathf.Clamp((mercy.transform.position.x-actor.Body.position.x)*2,-1,1),0):Vector2.zero};});Snapshot("passing-chapel");yield return Await("chapel returns within jumping distance",()=>chapel.transform.position.x<50.5f,20);yield return FrameJump(43.5f,25,true);}
        ...
        IEnumerator Run()
        {
            ...
            foreach(var stage in stages){if(stage==null){failures++;Note("FAIL unknown Fall stage "+selected);break;}
                if(Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-require-earned-world")>=0){bool earned=stage.boss?CampaignProgression.BossOpen(world,game.Save.Data,PracticeWitness):CampaignProgression.LevelOpen(world,Array.IndexOf(world.levels,stage),game.Save.Data,PracticeWitness);if(!earned){failures++;Note("FAIL Fall stage was not earned by the supplied real save");Finish();yield break;}}
            ...
            if((selected=="W4"||(selected=="GB-B4"&&Array.IndexOf(Environment.GetCommandLineArgs(),"-gb-require-earned-world")>=0))&&!stopped){var save=new SaveStore(Path.Combine(dir,"test-save.json"));Check("Fall earns the False Empyrean",CampaignProgression.WorldOpen(game.AvailableWorlds,4,save.Data,PracticeWitness));Check("sixteen Mercies cannot restore the ending",!save.RestoredEnding);}
            Finish();
        }
