    ...
    public sealed class ActorMotor : MonoBehaviour, IHittable
    {
        ...
        public bool SetSize(Vector2 size)
        {
            ...
            if(size.y>Shape.size.y || size.x>Shape.size.x)
            {
                ...
                for(int i=0;i<n;i++) if(overlaps[i]&&!overlaps[i].isTrigger&&overlaps[i]!=Shape){if(overlaps[i].GetComponent<OneWaySurface>()&&feet.y<overlaps[i].bounds.max.y-.12f)continue;return false;}
            }
